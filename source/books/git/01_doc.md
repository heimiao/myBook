...
- 先将你手头上的代码暂存到一个缓存区，缓存区包括你暂存区的代码和未提交到暂存区的代码
```
git stash

```
- 切换到你要处理的分支，处理临时安排的bugger问题
```
git checkout -b xxxx

```
- 档处理完成问题，切换回当前修改的分支进行释放缓存区
...
__一般在两个独立的仓库中，它们各自有自己的提交历史__

1. 你已经创建了一个新的仓库，并开始在其中工作。然后，你意识到你应该已经有一个现有的仓库，并且你想要将这两个仓库合并在一起。

2. 你可能在不同的分支上工作了一段时间，然后意识到你应该在同一个分支上工作。

3. 你可能在一个新的分支上工作，并且想要将它合并到主分支上。

**解决方案：**

- 首先，你需要确保你已经添加了远程仓库。如果你还没有添加，你可以使用以下命令
```
git remote add origin <repository_url>
```
- 然后，你可以使用以下命令来拉取远程仓库的所有内容：
```
git pull origin master --allow-unrelated-histories
```
- 最后，你可以将你的本地更改推送到远程仓库：
```
git push origin master
```
注意：在这个命令中，“origin”是远程仓库的名称，“master”是你想要合并的分支的名称。如果你的主分支有一个不同的名称，你需要将“master”替换为你的主分支的实际名称。